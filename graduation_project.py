...
def helper():
    ...
    print("------------")
    print("\t./graduation_project.py example.com: Scans the domain example.com.")
    print(
        "\t./graduation_project.py example.com --skip dmitry --skip theHarvester: Skip the 'dmitry' and 'theHarvester' tests.")
    print("\t./graduation_project.py example.com --nospinner: Disable the idle loader/spinner.")
    print("\t./graduation_project.py --update   : Updates the scanner to the latest version.")
    print("\t./graduation_project.py --help     : Displays this help context.")
    print(bcolors.OKBLUE + "Interactive:" + bcolors.ENDC)
    print("------------")
    print("\tCtrl+C: Skips current test.")
    print("\tCtrl+Z: Quits gradution project.")
...
class Spinner:
    ...
    def spinner_task(self):
        ...
        try:
            ...
            print(
                "\n\t" + bcolors.BG_ERR_TXT + "graduation project received a series of Ctrl+C hits. Quitting..." + bcolors.ENDC)
            sys.exit(1)
...
tool_cmd = [
    ...
    # 2
    ["wget -O /tmp/graduation_project_temp_aspnet_config_err --tries=1 ", "/%7C~.aspx"],

    # 3
    ["wget -O /tmp/graduation_project_temp_wp_check --tries=1 ", "/wp-admin"],

    # 4
    ["wget -O /tmp/graduation_project_temp_drp_check --tries=1 ", "/user"],

    # 5
    ["wget -O /tmp/graduation_project_temp_joom_check --tries=1 ", "/administrator"],
    ...
    # 75
    ["wget -O /tmp/graduation_project_temp_aspnet_elmah_axd --tries=1 ", "/elmah.axd"],
    ...
    # 78
    ["wapiti ", " -f txt -o graduation_project_temp_wapiti"],
...
def get_parser():
    ...
    parser.add_argument('-u', '--update', action='store_true',
                        help='Update graduation project.')
...
if args_namespace.help or (not args_namespace.update \
    ...
    logo()
    print("our graduation project is updating....Please wait.\n")
    spinner.start()
    # Checking internet connectivity first...
    rs_internet_availability = check_internet()
    if rs_internet_availability == 0:
        print(
            "\t" + bcolors.BG_ERR_TXT + "There seems to be some problem connecting to the internet. Please try again or later." + bcolors.ENDC)
        spinner.stop()
        sys.exit(1)
    cmd = 'sha1sum graduation_project.py | grep .... | cut -c 1-40'
    oldversion_hash = subprocess.check_output(cmd, shell=True)
    oldversion_hash = oldversion_hash.strip()
    os.system(
        'wget -N https://github.com/Sief-Ahmed/gradution-project/blob/main/graduation_project.py -O graduation_project.py > /dev/null 2>&1')
    ...
    if oldversion_hash == newversion_hash:
        clear()
        print("\t" + bcolors.OKBLUE + "You already have the latest version of graduation project." + bcolors.ENDC)
    else:
        clear()
        print("\t" + bcolors.OKGREEN + "graduation project successfully updated to the latest version." + bcolors.ENDC)
    spinner.stop()
    sys.exit(1)
    ...
    while (rs_avail_tools < len(tools_precheck)):
        ...
        try:
            ...
            print("\t" + bcolors.BG_ERR_TXT + "graduation project was terminated abruptly..." + bcolors.ENDC)
            sys.exit(1)
        ...
        if b"not found" in val or tools_precheck[rs_avail_tools][arg1] in args_namespace.skip:
            if b"not found" in val:
                print("\t" + bcolors.OKBLUE + tools_precheck[rs_avail_tools][
                    arg1] + bcolors.ENDC + bcolors.BADFAIL + "...unavailable." + bcolors.ENDC)
            elif tools_precheck[rs_avail_tools][arg1] in args_namespace.skip:
                print("\t" + bcolors.OKBLUE + tools_precheck[rs_avail_tools][
                    arg1] + bcolors.ENDC + bcolors.BADFAIL + "...skipped." + bcolors.ENDC)
            ...
            print("\t" + bcolors.OKBLUE + tools_precheck[rs_avail_tools][
                arg1] + bcolors.ENDC + bcolors.OKGREEN + "...available." + bcolors.ENDC)
        rs_avail_tools = rs_avail_tools + 1
        clear()
    unavail_tools_names = list(set(unavail_tools_names))
    if len(unavail_tools_names) == 0:
        print(
            "\t" + bcolors.OKGREEN + "All Scanning Tools are available. Complete vulnerability checks will be performed by graduation project." + bcolors.ENDC)
    else:
        print("\t" + bcolors.WARNING + "Some of these tools " + bcolors.BADFAIL + str(
            unavail_tools_names) + bcolors.ENDC + bcolors.WARNING + " are unavailable or will be skipped. graduation project will still perform the rest of the tests. Install these tools to fully utilize the functionality of graduation project." + bcolors.ENDC)
    ...
    while (tool < len(tool_names)):
        print("[" + tool_status[tool][arg3] + tool_status[tool][arg4] + "] Deploying " + str(tool + 1) + "/" + str(
            tool_checks) + " | " + bcolors.OKBLUE + tool_names[tool][arg2] + bcolors.ENDC, )
        ...
        temp_file = "/tmp/graduation_project_temp_" + tool_names[tool][arg1]
        ...
        if runTest == 1:
            ...
            print(bcolors.OKBLUE + "\nScan Completed in " + display_time(int(elapsed)) + bcolors.ENDC, end='\r',
                  flush=True)
            print("\n")
            # clear()
            ...
            print(bcolors.OKBLUE + "\nScan Interrupted in " + display_time(int(elapsed)) + bcolors.ENDC, end='\r',
                  flush=True)
            print(
                "\n" + bcolors.WARNING + "\tTest Skipped. Performing Next. Press Ctrl+Z to Quit graduation project.\n" + bcolors.ENDC)
            rs_skipped_checks = rs_skipped_checks + 1
    ...
    if len(rs_vul_list) == 0:
        ...
        with open("RS-Vulnerability-Report", "a") as report:
            while (rs_vul < len(rs_vul_list)):
                ...
                report.write("\n------------------------\n\n")
                temp_report_name = "/tmp/graduation_project_temp_" + vuln_info[arg1]
            ...
            print(
                "\tComplete Vulnerability Report for " + bcolors.OKBLUE + target + bcolors.ENDC + " named " + bcolors.OKGREEN + "`RS-Vulnerability-Report`" + bcolors.ENDC + " is available under the same directory graduation project resides.")
    ...
    for file_index, file_name in enumerate(tool_names):
        with open("RS-Debug-ScanLog", "a") as report:
            try:
                with open("/tmp/graduation_project_temp_" + file_name[arg1], 'r') as temp_report:
                    data = temp_report.read()
    ...
    print("\tTotal Number of Vulnerability Checks        : " + bcolors.BOLD + bcolors.OKGREEN + str(
        len(tool_names)) + bcolors.ENDC)
    print("\tTotal Number of Vulnerability Checks Skipped: " + bcolors.BOLD + bcolors.WARNING + str(
        rs_skipped_checks) + bcolors.ENDC)
    print("\tTotal Number of Vulnerabilities Detected    : " + bcolors.BOLD + bcolors.BADFAIL + str(
        len(rs_vul_list)) + bcolors.ENDC)
    print("\tTotal Time Elapsed for the Scan             : " + bcolors.BOLD + bcolors.OKBLUE + display_time(
        int(rs_total_elapsed)) + bcolors.ENDC)
    print("\n")
    print(
        "\tFor Debugging Purposes, You can view the complete output generated by all the tools named " + bcolors.OKBLUE + "`RS-Debug-ScanLog`" + bcolors.ENDC + " under the same directory.")
    print(bcolors.BG_ENDL_TXT + "[ Report Generation Phase Completed. ]" + bcolors.ENDC)

    os.system('setterm -cursor on')
    os.system('rm /tmp/graduation_project_te* > /dev/null 2>&1')  # Clearing previous scan files
